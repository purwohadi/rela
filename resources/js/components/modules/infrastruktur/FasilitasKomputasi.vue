<template>
	<div class="row">
		<div class="col-12">
			...
			<section class="fasilitas-komputasi">
				<data-tables
					:id="`${tableId}`"
					:ref="`${tableId}`"
					:fields="fields"
					:api-url="'fasilitas.data-domain-infra-fasilitas.get'"
					:args-route="{'opd_id': opd_id}"
					:title="$t('infrastruktur.datatable.titleFasilitasKomputasi')"
					actions="SCL"
					:access="action"
					row-action-width="10%"
					@add="handleAdd">  
...
export default {
	...
	methods: {
		...
		handleUbah(data){
			this.$router.push({name: 'infrastruktur.ubah-fasilitas-komputasi', params: {
				'id': data.slug_path,
				'status': 'edit'
			}})
		},
		handleDetil(data){
			this.$router.push({name: 'infrastruktur.detil-fasilitas-komputasi', params: {
				'id': data.slug_path,
				'status': 'detail'
			}})
		},
		handleAdd() {
			this.$router.push({name: 'infrastruktur.tambah-fasilitas-komputasi', params: {
				opd_id: this.model.perangkatDaerah.opd_id
			}})
		},
		handleDelete(data){
			let vm 			= this
			vm.$app.confirm({
				...
				preConfirm: () => {
					return vm.$http
						.post(vm.$app.route('fasilitas.delete-data-domain-infra-fasilitas'), {
							data: { id: data.id}
						})
						.then(response => {
