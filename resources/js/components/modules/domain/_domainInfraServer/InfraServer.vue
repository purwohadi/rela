<template>
	<div class="row">
		<div class="col-12">
			...
			<section class="infra-server">
				<data-table
					:id="`${resourceName}`"
					:ref="`${resourceName}`"
					:api-url="'domain-infra-server.list'"
					:fields="fields"
					:urlParams="{'opd_id': opd_id}"
					:title="$t('domaininfra.datatable.title', {domain: 'Server'})"
...
export default {
	...
	data() {
		return {
			...
			fields: [
				...
				{
					key: 'nama',
					label: this.$t('domaininfra.datatable.column.name', {name: 'Server'}),
					sortable: true,
				},
				{
					key: 'instansi',
		...
		'model.perangkatDaerah': {
			handler: function(n) {
				if (!_.isEmpty(n)) {				
					...
					this.$app.user.can('infra-server') ? 'C' : ''
				}
			},
		},
	},
	computed: {
		actionsAplikasi() {
			let actions = 'SR'
			if(this.opd_id){
				actions += this.$app.user.can('infra-server') ? 'C' : ''
			}
      		return actions
		}
	},
