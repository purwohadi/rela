<template>
	<div class="row">
		<div class="col-12">
			<section id="peningkatan-keamanan-form">						
					...
					<ValidationObserver v-slot="{ passes }" :slim="true">
						<form @submit.prevent="passes(onSubmit)">
							...
							<b-form-group
								label-cols="2"
								label-cols-lg="2"
								label-for="id-metadata-terkait-app"
								label="ID Metadata Terkait (Aplikasi)">
								<b-col sm="6">
									<validation-provider name="ID Metadata Terkait (Aplikasi)" rules="required" v-slot="{ errors, ariaMsg }">
										<m-select
											id="id-metadata-terkait-app"
											ref="id-metadata-terkait-app"
											class="mb-2"
											:option-height="73"
											:tabindex="7"
											:options="optionsIdMetadataTerkaitApp"
											v-model="model.id_metadata_terkait_app"
											:multiple="true"
							...
							<b-form-group
								label-cols="2"
								label-cols-lg="2"
								label-for="id-metadata-terkait-netdev"
								label="ID Metadata Terkait (Netdev)">
								<b-col sm="6">
									<validation-provider name="ID Metadata Terkait (Netdev)" rules="required" v-slot="{ errors, ariaMsg }">
										<m-select
											id="id-metadata-terkait-netdev"
											ref="id-metadata-terkait-netdev"
											class="mb-2"
											:option-height="73"
											:tabindex="7"
											:options="optionsIdMetadataTerkaitNetdev"
											v-model="model.id_metadata_terkait_netdev"
											:multiple="true"
											:close-on-select="false" 
											:clear-on-select="false" 
											:preserve-search="true" 
											:preselect-first="true"
											:custom-label="nameSelectedKode"
											select-label=""
											selected-label=""
											deselect-label=""
											track-by="kode"
...
export default
{
	...
	data(){
		return{				
			...
			model: {				
				...
				rai_level_2_fix: '02.03 Peningkatan Keamanan SPBE',
				rai_level_1: [],
				rai_level_2: [],
				id_metadata_terkait_app: [],
				id_metadata_terkait_netdev: [],
			...
			optionsRaiLevel1: [],
			optionsRaiLevel2: [],
			optionsIdMetadataTerkaitApp:[],
			optionsIdMetadataTerkaitNetdev:[],
			isDetail: false,
		}
	},
	computed: { },
	...
	created(){
		...
		this.resDomainPeningkatanKeamanan()
		this.isDetail = (this.model.status == 'detail')?false:true		
		this.resRal('', 1).then((data) =>{this.optionsRaiLevel1 = data})
		this.resMetadataApp().then((data) => {this.optionsIdMetadataTerkaitApp = data})
		this.resMetadataNetdev().then((data) => {this.optionsIdMetadataTerkaitNetdev = data})
		console.log(this.resMetadataNetdev())
	},
	methods :
	{				
		onBack() {
		...
		onSubmit(){
			let vm 			= this			
      		let routeName 	= vm.model.status ? 'peningkatan.update-data-domain-peningkatan-keamanan' : 'peningkatan.simpan-data-domain-peningkatan-keamanan'
		...
		async resDomainPeningkatanKeamanan(){
			const res = await this.$http.get(this.$app.route('peningkatan.search-peningkatan-keamanan.get', {id_peningkatan: this.model.id}))
			...
			if(res.data){
				...
				//get data aplikasi
				const metadata = res.data.id_metadata_terkait_app
				let split = metadata.split("; ")
				
				let isi_app = []
				for (let element of split) { // You can use `let` instead of `const` if you like
					let a = this.optionsIdMetadataTerkaitApp.find(data => data.kode === element)
					isi_app.push(a)					
				}
				this.model.id_metadata_terkait_app = isi_app

				//get data Netdev
				const metadata2 = res.data.id_metadata_terkait_netdev
				let split_netdev = metadata2.split("; ")
				
				let isi_netdev = []
				for (let element of split_netdev) { // You can use `let` instead of `const` if you like
					let b = this.optionsIdMetadataTerkaitNetdev.find(data => data.kode === element)
					isi_netdev.push(b)					
				}
				this.model.id_metadata_terkait_netdev = isi_netdev
		...
		nameSelectedKode({kode}){
			return `${kode}`
		},
		async resMetadataApp(){
			const res = await this.$http.get(this.$app.route('peningkatan.get-metadata-app'))
			return res.data			
		},
		async resMetadataNetdev(){
			const res = await this.$http.get(this.$app.route('peningkatan.get-metadata-netdev'))
			return res.data			
		},  
	},
}
