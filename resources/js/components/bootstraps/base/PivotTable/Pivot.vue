<template>
  <div>
    ...
    <div class="mb-4" v-else>
      <button class="btn btn-outline-primary btn-sm" @click="toggleShowSettings">{{ showSettingsText }}</button>
      <button v-if="adaTableId" class="btn btn-outline-primary btn-sm" style="float: right;" @click="downloadExcel" :disabled="isDataLoading">Download</button>
      <!-- <button v-if="exportData" class="btn btn-outline-primary btn-sm mr-2" style="float: right;" @click="$emit('onExport')" :disabled="false">Export</button> -->
    </div>
...
export default {
  ...
  computed: {
    adaTableId: function() {
      if (this.tableId == null || this.tableId == '') return false
      return true
    },
  ...
  methods: {
    ...
    downloadExcel: function() {
			...
			XLSX.writeFile(wb, this.tableId +"-dashboard.xlsx");
		},
    downloadData() {
			// var wb = XLSX.utils.table_to_book(document.getElementById(this.tableId));
			// XLSX.writeFile(wb, this.tableId +"-pkt-dashboard.xlsx");
      this.$emit('onExport')
		},
    start: function() {
      this.dragging = true
