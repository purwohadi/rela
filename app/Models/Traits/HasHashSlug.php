...
trait HasHashSlug {
  ...
  {
    $this->appends = array_unique(array_merge($this->appends, ['slug_path']));
	...
	public function slug(){
		if (is_null($this->slug)){
			...
			$id = bin2hex($this->{$this->getKeyName()});
			$this->slug = $hashids->encodeHex($id);
		}
  ...
  {
    return method_exists($this, 'slug') ? $this->slug() : $this->attributes['id'];
  }
